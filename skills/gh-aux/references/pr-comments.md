...
| `gh aux pr-comments update`         | Update the body of an inline review comment                | `--id`, `--body`          |
...
- `update` → updated inline comment object: `{ id, body, path, line, originalLine, author: { login }, createdAt, updatedAt, url }`
...
- `event` — optional. Omit to leave the review in pending state. If you already have a pending review open, comments are appended to it; otherwise a new pending review is created.
- `body` — requires `event`. Top-level review comment submitted alongside the event (matches the GitHub UI "Submit review" flow).
...
- `side` — required for line-level comments; must be uppercase (`"LEFT"` or `"RIGHT"`).
- `start_side` — required when `start_line` is set. Must be uppercase (`"LEFT"` or `"RIGHT"`).
